...
struct MemoryGame<CardContent> where CardContent: Equatable {
    ...
    mutating func choose(_ card: Card) {
        if let chosenIndex = cards.firstIndex(where: { $0.id == card.id }){
            if !cards[chosenIndex].isFaceUp && !cards[chosenIndex].isMatched {
                if let potentielMatchIndex = indexOfTheOneAndOnlyFaceUpCard {
                    if cards[chosenIndex].content == cards[potentielMatchIndex].content {
                        cards[chosenIndex].isMatched = true
                        cards[potentielMatchIndex].isMatched = true
                        score += 2 + cards[chosenIndex].bonus + cards[potentielMatchIndex].bonus
                    } else {
                        if cards[chosenIndex].hasBeenSeen {
                            score -= 1
                        }
                        if cards[potentielMatchIndex].hasBeenSeen {
                            score -= 1
                        }
                    }
                } else {
    ...
    struct Card: Equatable, Identifiable, CustomDebugStringConvertible {
        ...
        //can be zero which would mean "no bonus available" for matching this card quicly
        ...
        //(i.e. not including the current time it's been face up if is is currently so)
